...
export class CitiesService {
  ...
  async fetchCitiesFromAPI(): Promise<City[]> {
    const citiesMap = new Map<string, City>();
    const urls = Array.from(
      { length: 3 },
      (_, i) =>
        `https://hubeau.eaufrance.fr/api/v1/qualite_eau_potable/communes_udi?size=20000&fields=code_commune%2Cnom_commune&page=${i + 1}`
    );
