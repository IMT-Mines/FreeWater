...
export class ApiSamples {
  async fetchSampleFromCity(code: string): Promise<SampleData> {
    try {
      ...
      const sampleData: SampleData = {
        ...
        drinkableScore: this.getDrinkableScore(response.data.data[0].conclusion_conformite_prelevement),
        samples: []
      };
  ...
  async getAllSamples(codesCities: string[]) {
    const sampleData: SampleData[] = [];
    for (const codeCity of codesCities) {
      sampleData.push(await this.fetchSampleFromCity(codeCity));
    }
    return sampleData;
  }

  getDrinkableScore(conformity: string) {
    if (
      conformity ===
      ...
      conformity === "Eau d'alimentation conforme aux limites de qualité et non conforme aux références de qualité." ||
